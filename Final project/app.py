    ...
    if request.method == "POST":
        ...
        if acc:
            if (str(cpass)) == str(acc['PASSWORD'].strip()):
                ...
                session["login_email"] = ""+str('admin@gmail.com')
...
def addsignupuser():
    ...
    if request.method == "POST":
        ...
        if acc:
            ...
            from_email = Email("mail","CUSTOMER CARE REGISTRY")
...
def addsignupadmin():
    if request.method == "POST":
        ...
        if acc:
            return render_template("login_admin.html",msg="Your are already our admin,Please Login!", admin_email=admin_email)
        else:
            global admin_rand
            ...
            to_email = To(user_email)
            subject = "Verification( USER )"
            content = Content("text/plain", "Hi "+user_fullname+" , This is your verification code : "+str(admin_rand))
            mail = Mail(from_email, to_email, subject, content)
            sg = sendgrid.SendGridAPIClient('your api key')
            response = sg.client.mail.send.post(request_body=mail.get())
    ...
    if (logintype==None or loginemail==None) :
        ...
        if request.method =='POST':
            ...
            while query:
                qlist.append(query)
                query = ibm_db.fetch_both(stmt)
            ema_sql = "SELECT AGENTEMAIL FROM CUSTOMERQUERIES WHERE TICKET_ID=? "
...
def againquery():
    if request.method == "POST":
        ...
        status  = 'pending'
        whois = 'customer'
        up_sql = "UPDATE CUSTOMERQUERIES SET STATUS = ? , QUERY =? WHERE TICKET_ID = ?;"
...
def adminsinglepage():
    ...
    if (logintype==None or loginemail==None) :
        ...
        if request.method == "POST":
            ...
            if(adminkey=="handle"):
                messages = "NotAssign"
                all_sql = "SELECT * FROM CUSTOMERQUERIES WHERE AGENTEMAIL=?"
...
def updatedagent():
    if request.method =="POST":
        ...
        #Query for updating the Status in the CUSTOMERQUERIES Table
        up_agent_sql = "UPDATE CUSTOMERQUERIES SET AGENTEMAIL = ? WHERE TICKET_ID = ?;"
...
def addinviteagents():
    if request.method =="POST":
        ...
        if acc:
            ...
            from_email = Email("ibmcustomerreg@gmail.com","CUSTOMER CARE REGISTRY")
...
def agentsinglepage():
    ...
    if (logintype==None or loginemail==None) :
        ...
        if request.method == "POST":
            ...
            all_sql = "SELECT * FROM CUSTOMERQUERIES WHERE STATUS = ? AND AGENTEMAIL = ?"
            allmt = ibm_db.prepare(conn,all_sql)
            ibm_db.bind_param(allmt, 1, messages)
